...
def compute_f0_praat(audio_file, input_parms=None):
    ...
    parms["pitch_floor"] = check_pitch_parms(
        parms["pitch_floor"], parms["time_step"])
    ...
    praat_command = "\"C:\\praat\\Praat.exe\" --run %s %s %s %.10f %f %g %g %.10f %.10f %.10f %.10f %.10f %f" % (praat_script_path, audio_file, pitch_file_name, parms["time_step"], parms["pitch_floor"], parms["max_n_candidates"], parms[
        "very_accurate"], parms["silence_threshold"], parms["voicing_threshold"], parms["octave_cost"], parms["octave_jump_cost"], parms["voiced_unvoiced_cost"], parms["pitch_ceiling"])
